...
def sentences2bow(text):
    ...
    word2id = vectorizer_1.vocabulary_
    id2word = {}
    for k in word2id:
        id = word2id[k]
        id2word[id] = k
    ...
    bows_all_words = torch.from_numpy(bows_all_words)
    words_cnt = bows_all_words.sum(dim=0)
    val, idx = torch.topk(words_cnt, k=2000)
...
scio.savemat(os.path.join(root_dir, "caption_one_hot.mat"), {'caption_one_hot': txt_one_hot})
